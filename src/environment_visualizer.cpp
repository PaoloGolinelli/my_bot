...
public:
    ...
    void spawnBoxes(const vector<Box> &boxes) {
        for (const auto &box : boxes) {
            ...
            while (!spawn_client_->wait_for_service(chrono::seconds(1))) {
    ...
    void despawnBoxes(const vector<string> &box_names) {
        for (const auto &name : box_names) {
            auto request = make_shared<gazebo_msgs::srv::DeleteEntity::Request>();
            request->name = name;

            while (!delete_client_->wait_for_service(chrono::seconds(1))) {
...
private:
    ...
    string createBoxSDF(const Box &box) {
        return R"(
            <sdf version="1.6">
              <model name=")" + box.name + R"(">
                ...
                <link name="link">
                  <collision name="collision">
                    <geometry>
                      <box>
                        <size>)" + to_string(box.size_x) + " " + to_string(box.size_y) + " " + to_string(box.size_z) + R"(</size>
                      </box>
                    </geometry>
                  </collision>
                  <visual name="visual">
...
int main(int argc, char **argv) {
    ...
    auto node = make_shared<BoxSpawner>();
